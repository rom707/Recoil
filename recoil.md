...
### Вступление
Recoil - библиотека управления состоянием React-приложений созданная командой из Facebook и основанная на графе потока данных.
...
Так как общий родитель компонентов находится на несколько узлов выше, логично использовать React Context для общего состояния.
...
Либо использовать `useRecoilValueLoadable()` для того, чтобы обработать ожидание самостоятельно:
```js
function UserInfo({userID}) {
...
Для того, чтобы использовать Recoil, необходимо обернуть приложение провайдером `RecoilRoot`. Есть поддержка дефолтного состояния:
```js
function MyApp() {
...
Другое преимущество Recoil — простота использования. Основное взаимодействие с Recoil происходит через использования хуков напоминающих `React useState`. Также нет надобности писать большое количество шаблонного кода в отличие от Redux.

Однако в остальном библиотека Recoil пока что не может похвастаться тем выбором утилит и дополнительных инструментов созданных под Redux, а также сообществом и опытом использования в реальных проектах.
...
Несмотря на это, у библиотеки есть шансы получить поддержку и популярность хотя бы за счёт проблем, которые она решает. Библиотека уже набрала 12k звезд на Github и имеет 80k скачиваний за неделю, что говорит об активном интересе со стороны разработчиков. Также, по словам автора, планируется использование `React Concurrent Mode`, что является плюсом.
